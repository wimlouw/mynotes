...
(still editing this post for language)
...
This is my main example: getting some minimal job information from a site called Gumtree.co.za.
...
I code another function, which will create a tibble[^14] for every job post, with the information I want. I can then put this into a list of tibbles using a for loop. 
...
[^16]: See "safely" and "possibly" from the [purrr](https://purrr.tidyverse.org/) package (or "trycatch" from base R)
...
all <- all[!is.na(all)] ##remove empty lists, if any
all <- bind_rows(output) %>% ##Fab!
glimpse(all)
