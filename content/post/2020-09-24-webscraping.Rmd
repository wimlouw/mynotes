...
I used a combination of the inspect tool and [SelectorGadget](http://selectorgadget.com/) ("point and click CSS selectors") to identify what the elements I want to extract are called. 

I used the [rvest](https://github.com/tidyverse/rvest) R package ("rvest helps you scrape information from webpages"), and a number of [tidyverse tools](https://www.tidyverse.org/) for data manipulation. Code is heavily commented. 
...
In the first example, I extract a table from Wikipedia, using `rvest`'s `html_table()` command. 
...
This is my main example :P 

I will be getting some minimal job information from a site called Gumtree.co.za.
...
I want to get the links on the first page:
...
Then, I want to zoom in to one ad and see what's in there. 

I decided to extract the job title, description, job type, employer, location, and also include a time-stamp. I used the inspect tool to identify each element's CSS-selector. 

(There's much more information on this page than the few elements I chose for this example, but the approach will be the same if I wanted to include more.)
...
Since I wanted to do this for a few more things --- title, description, &c., I made a small function to make that easier, and to avoid repeating myself too much:
...
OK this worked fine. But what if I wanted to do this for all job posts on a page?

#### Iteration!

I wrote another function, which will create a tibble[^14] for every job post, with the information I want. I can put this into a list of tibbles using a for loop. After I have a list of tibbles, I can combine them into a single dataset.
...
In the example below, I extracted the job links for the first 3 pages:
